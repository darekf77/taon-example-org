    ...
    <taon-admin-mode-configuration>
      @if (itemsLoaded()) {
        ...
        <footer
          ...
          Copyright <strong>share</strong> {{ year }}
        </footer>
      }
    </taon-admin-mode-configuration>
  `,
...
export class ShareApp implements OnInit {
  ...
  navItems =
    ShareClientRoutes.length <= 1
      ? []
      : ShareClientRoutes.filter(r => r.path !== undefined).map(r => ({
          path: r.path === '' ? '/' : `/${r.path}`,
...
//#region  share routes
//#region @browser
export const ShareServerRoutes: ServerRoute[] = [
  {
    path: '**',
    renderMode: RenderMode.Prerender,
  },
];
export const ShareClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (ShareClientRoutes.length === 1) {
        return '';
      }
      return ShareClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
//#region  share app configs
//#region @browser
export const ShareAppConfig: ApplicationConfig = {
  providers: [
    provideZonelessChangeDetection(),
    {
      provide: TAON_CONTEXT,
      useFactory: () => ShareContext,
    },
    providePrimeNG({
      theme: {
        preset: Aura,
...
export const ShareServerConfig: ApplicationConfig = {
  providers: [provideServerRendering(withRoutes(ShareServerRoutes))],
};

export const ShareConfig = mergeApplicationConfig(
  ShareAppConfig,
  ShareServerConfig,
);
//#endregion
...
var ShareContext = Taon.createContext(() => ({
  ...HOST_CONFIG['ShareContext'],
...
//#region  share start function
export const ShareStartFunction = async (
...
export default ShareStartFunction;
//#endregion
